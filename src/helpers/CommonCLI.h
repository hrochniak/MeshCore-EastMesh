...
public:
  ...
  uint8_t extra_sf[4];
  // EastMesh-only fields
  uint8_t fan_mode = 0;
  uint16_t fan_timeout_secs = 0;
  char bridge_peer_host[64]; // peer MQTT broker host (MQTT bridge only)
  uint16_t bridge_peer_port = 0;   // peer MQTT broker port (MQTT bridge only, default 1883)
...
private:
  ...
  protected:
    void structure() override {
      ...
      def("secret", _parent->bridge_secret, sizeof(_parent->bridge_secret)); // for XOR encryption of bridge packets (ESP-NOW / MQTT)
      // EastMesh: peer MQTT broker (MQTT bridge only)
      def("peer_host", _parent->bridge_peer_host, sizeof(_parent->bridge_peer_host));
      def("peer_port", _parent->bridge_peer_port);
      def("peer_user", _parent->bridge_peer_username, sizeof(_parent->bridge_peer_username));
      def("peer_pwd", _parent->bridge_peer_password, sizeof(_parent->bridge_peer_password));
    }
  public:
  ...
  class FanPrefs : public ConfigSerializer {  // EastMesh: TBeam-1W fan control
    NodePrefs* _parent;
  protected:
    void structure() override {
      def("mode", _parent->fan_mode);      // 0 = auto, 1 = on, 2 = off
      def("timeout", _parent->fan_timeout_secs);
    }
  public:
    FanPrefs(NodePrefs* parent) : _parent(parent) { }
  };
  FanPrefs fan;
...
public:
  NodePrefs() : ConfigSerializer(), bridge(this), gps(this), radio(this), power(this), repeat(this), room(this), fan(this) {
    node_name[0] = 0;
    password[0] = 0;
    guest_password[0] = 0;
    bridge_secret[0] = 0;
    owner_info[0] = 0;
    bridge_peer_host[0] = 0;
    bridge_peer_username[0] = 0;
    bridge_peer_password[0] = 0;
  }
};
