...
public:
  ...
  uint8_t wifi_powersave = 0;    // 0 = none, 1 = min, 2 = max
  char wifi_ssid[33];
  char wifi_pwd[65];
...
private:
  ...
  class WifiPrefs : public ConfigSerializer {  // EastMesh: companion WiFi settings
    NodePrefs* _parent;
  protected:
    void structure() override {
      def("psave", _parent->wifi_powersave);   // 0 = none, 1 = min, 2 = max
...
public:
  NodePrefs() : radio(this), gps(this), wifi(this), companion(this) {
    node_name[0] = 0;
    wifi_ssid[0] = 0;
    wifi_pwd[0] = 0;
